...
class _ViewPasswordPage extends State<ViewPasswordPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (Platform.isAndroid) {
      detailWidgets.add(_titleContainer(mainColor, "所属App"));
          ...
          children: <Widget>[
            Expanded(
              child: InkWell(
                ...
                child: Text(
                  bean.appId?.isEmpty ?? true ? "无" : bean.appName!,
                  overflow: TextOverflow.ellipsis,
                  maxLines: 1,
                ...
                onTap: () {
                  if (bean.notes.length >= 1) {
                    Navigator.push(
                      context,
                      CupertinoPageRoute(
                        builder: (_) => DetailTextPage("备注", bean.notes, false),
                      ),
                    );
                  }
                },
                child: Text(
                  bean.notes.length < 1 ? "无备注" : bean.notes,
                  overflow: TextOverflow.ellipsis,
                  maxLines: 2,
