...
class DefaultFolderPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _getWidgetListView(context, _controller),
    );
  }

  Widget _getWidgetListView(BuildContext context, ScrollController controller) {
    List<Widget> list1 = [];
    List<Widget> list2 = [];
    List<Widget> all = [];
    ...
    for (int index = 0; index < provider1.count; index++) {
      try {
        var password = provider1.passwordList[index];
        if (password.folder.isEmpty) {
    ...
    for (int index = 0; index < provider2.count; index++) {
      try {
        var card = provider2.cardList[index];
